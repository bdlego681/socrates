...
colors:
  ...
  accent-sunset: "#FF8C4F"
  highlight-lime: "#C8E53C"
  shadow-blue: "#172140"
...
Socrates's design language opens with the gradient mesh. A wide horizontal band of Accent Sunset, Highlight Lime, Secondary Teal, and Primary Indigo occupies the upper third of nearly every marketing page — the brand's instantly-recognizable atmospheric backdrop. Type and product UI mockups float above it on `{colors.canvas}` (white), with the gradient acting as both decoration and visual anchor. The lower portion of the page returns to white, with feature explanations on `{colors.canvas-soft}` (a barely-tinted cool off-white) and dashboard product mockups composited as faux IDE/console panels in deep Primary Indigo.

The color system has two primary roles. **Secondary Teal** (`{colors.primary}` — `#3AB9B0`) is the brand's signature CTA color, used sparingly: one filled pill per band. **Primary Indigo** (`{colors.brand-dark-900}` — `#233261`) heavily defines the dashboard track, acting as the universal body text color (`{colors.ink}` variants) and the fill of dashboard sidebars, the featured pricing tier, and the dark-app surfaces. Accent Sunset (`{colors.accent-sunset}`) and Highlight Lime (`{colors.highlight-lime}`) appear inside the gradient mesh and as accent dots in product UI mockups; they are not used as button colors.
...
**Key Characteristics:**
- Gradient-mesh backdrop on every marketing hero — sunset/lime/teal/indigo horizontally washed across the upper third of the page.
...
- Dark-app dashboard track: deep Primary Indigo product UI mockups sit composited above the white canvas, frequently with rendered code or dashboard tables inside.
- Pill-shaped buttons (`{rounded.pill}` 9999px) with tight `8px 16px` padding — short, decisive, transactional.
- Cream-band feature cards (`{colors.canvas-cream}`) introduce a warm interlude between teal/white sections without breaking the brand's chromatic logic.

## Colors
> **Source pages:** home (`/`), `/data`, `/pricing`, `dashboard.socrates.com/register`.

### Brand & Accent
- **Secondary Teal** (`{colors.primary}` — `#3AB9B0`): The brand's signature CTA color. Filled-pill button, link emphasis, gradient anchor.
- **Teal Deep** (`{colors.primary-deep}` — `#2e968f`): A deeper teal used in gradient mid-stops and as the press-state warmer alternative.
- **Teal Press** (`{colors.primary-press}` — `#24756f`): Pressed-state lift of the primary.
- **Teal Soft** (`{colors.primary-soft}` — `#66c9c3`): A lighter teal used in product-UI accents and chart highlights.
- **Teal Subdued** (`{colors.primary-bg-subdued-hover}` — `#dcf2f0`): Pale teal fill used as soft tag background.
- **Primary Indigo** (`{colors.brand-dark-900}` — `#233261`): The deep navy used on the featured pricing tier and dashboard chrome.
- **Accent Sunset** (`{colors.accent-sunset}` — `#FF8C4F`): Gradient accent and chart highlight; never a button.
- **Highlight Lime** (`{colors.highlight-lime}` — `#C8E53C`): Brighter lime stop in gradient meshes.

### Surface
- **Canvas** (`{colors.canvas}` — `#ffffff`): Default page background.
- **Canvas Soft** (`{colors.canvas-soft}` — `#f6f9fc`): Cool-tinted off-white used on feature bands beneath the gradient hero.
- **Canvas Cream** (`{colors.canvas-cream}` — `#fff5f0`): Warm cream (tinted from Sunset) used as a feature-band fill — the brand's chromatic interlude.
...
### Text
- **Ink** (`{colors.ink}` — `#172140`): Default body text color across the brand. Deepest shade of indigo, never pure black.
- **Ink Secondary** (`{colors.ink-secondary}` — `#233261`): Secondary text on white, matching Primary Indigo.
...
- **On Primary** (`{colors.on-primary}` — `#ffffff`): Text on teal / dark-indigo surfaces.
...
- **Tabular figures for money.** Any cell rendering currency, transaction amounts, or numeric counts uses `font-feature-settings: "tnum"` plus a tightening tracking. The brand quietly signals its data DNA through this micro-detail.
...
| 1 | `box-shadow: rgba(23,33,64,0.08) 0 1px 3px` | Card lift on white |
| 2 | `box-shadow: rgba(23,33,64,0.08) 0 8px 24px, rgba(23,33,64,0.04) 0 2px 6px` | Floating panels, dashboard mockup chrome |
...
- Background `{colors.brand-dark-900}`, text `{colors.on-primary}`, otherwise identical structure to `card-pricing`. The deep-indigo fill is the brand's distinctive featured-tier choice.

**`card-cream-band`** — warm interlude card.
- Background `{colors.canvas-cream}`, text `{colors.ink}`, padding `{spacing.xxl}`, rounded `{rounded.lg}`. Used to break up the teal / white rhythm with warmth.
...
**Gradient Mesh Backdrop** — Accent Sunset → Highlight Lime → Secondary Teal → Primary Indigo stops blurred horizontally across the upper third of the page. Implemented as SVG or a large background image — not a flat CSS gradient (the real mesh has organic blob shapes).
...
**Tabular-Figure Money Type** — every number rendering money, count, or transaction value uses `font-feature-settings: "tnum"`. The brand's quiet signal that it's a data-infrastructure platform.
...
- Don't add new accent colors outside the documented gradient stops (sunset / lime / teal / indigo).
