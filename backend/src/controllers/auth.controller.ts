...
export async function logout(req:Request,res:Response){const t=token(req);if(t){const user = await getSessionUser(t, 'authenticated') || await getSessionUser(t, 'mfa_pending');if(user)await ActivityService.logActivity(user.id, 'logout');await invalidateSession(t);}clear(res);return res.status(204).send();} export const me=(req:Request,res:Response)=>res.json({user:req.user});
...
export async function enableMfa(req:Request,res:Response){
  ...
  await ActivityService.logActivity(req.user!.id, 'mfa_enabled');
...
export async function disableMfa(req:Request,res:Response){if(!await confirm(req))return res.status(401).json({error:'Unable to disable MFA'});await pool.request().input('id',sql.UniqueIdentifier,req.user!.id).query('UPDATE dbo.users SET mfa_enabled=0,mfa_secret_encrypted=NULL WHERE id=@id; DELETE FROM dbo.recovery_codes WHERE user_id=@id; DELETE FROM dbo.sessions WHERE user_id=@id');clear(res); await ActivityService.logActivity(req.user!.id, 'mfa_disabled'); return res.status(204).send();}
